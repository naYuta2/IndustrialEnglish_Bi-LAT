...
== III-A Overview
Bi-ACTは双方向制御を活用し位置情報と力情報の両方を用いたロボット制御を行う
- 未知の物体に対しても適応
...
自然言語指示を分散表現に変換するため、CLIPシリーズとBERTシリーズから計4種類の言語エンコーダを採用
...
- 学習は双方向制御オペレーション中に収集された実際のリーダー関節データとの誤差を最小化する事で行われる
...
出力は双方向制御システムの計算により各関節に必要な電流値に変換される
...
- 4チャンネル双方向制御システムにより1000Hzでリーダー・フォロワー両ロボットの関節角度・角速度・トルクを記録
...
結果はモデルが自然言語指示に基づいて力を調整する能力を持つ事を示しており、双方向制御ベースの模倣学習フレームワークに言語的手がかりを統合する事の有効性を実証している
